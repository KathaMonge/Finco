...
@ParserRegistry.register("mastercard")
class MastercardParser(BaseParser):
    DETECT_PATTERNS = [
        re.compile(r"mastercard", re.IGNORECASE),
        re.compile(r"master\s*card", re.IGNORECASE),
        re.compile(r"mc\s*\d{4}", re.IGNORECASE),
    ]

    AMOUNT_PATTERNS = [
        re.compile(r"(?:total|importe|monto)\s*:?\s*\$?\s*([\d.,]+)", re.IGNORECASE),
        re.compile(r"\$?\s*([\d.,]+)\s*$", re.MULTILINE),
    ]

    DATE_PATTERNS = [
        re.compile(r"(?:fecha|date)\s*:?\s*(\d{2}[/-]\d{2}[/-]\d{2,4})", re.IGNORECASE),
    ...
    def parse(self, lines: list[str]) -> OCRResult:
        result = OCRResult(emisor="mastercard", raw_lines=lines)
